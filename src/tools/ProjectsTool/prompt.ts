...
// PROJECTS_PROMPT is the verbatim description()/prompt() text (minified t4q),
// decoded from the `.bun` section. description() and prompt() both return it.

export const PROJECTS_PROMPT = `Read and write the claude.ai Project attached to this session. A Project is a shared knowledge container on claude.ai — its docs persist across sessions and surfaces (chat, Cowork, Claude Code), so anything you write here is visible to the user and their team in claude.ai.
